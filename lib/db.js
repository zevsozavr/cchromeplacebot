...
export async function upsertChannelProduct(product) {
  ...
  if (idx === -1) {
    ...
    const existingSizesValid = existing.sizes && existing.sizes.length > 0 && !(existing.sizes.length === 1 && existing.sizes[0] === 'One Size');
    appData.products[idx] = {
      ...
      sizes: existingSizesValid ? existing.sizes : product.sizes,
      sizeStock: existingSizesValid ? existing.sizeStock : product.sizeStock,
    };
  }
...
export async function saveOrder(order) {
  ...
  // Upsert: replace if same id already exists (e.g. retry after TTN update)
