...
async function edgeRead() {
  ...
  // Prefer Management API (always fresh, bypasses CDN cache)
  if (VERCEL_TOKEN) {
    try {
      const res = await fetch(`${MGMT_API}/${EC_ID}/items`, {
        headers: { 'Authorization': `Bearer ${VERCEL_TOKEN}` },
      });
      if (!res.ok) throw new Error(`mgmt ${res.status}`);
      const body = await res.json();
      // Management API returns { items: [{ key, value }, ...] } — convert to flat
      const flat = {};
  ...
  // Fallback to CDN read with token
  if (!EC_TOKEN) return null;
  try {
...
export async function edgeWrite(key, value) {
...
export async function saveOrder(order) {
  ...
  localFallback = null;
  const all = (await getAll()) || {};
  const orders = (all.orders || []).concat(order);
